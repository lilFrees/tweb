...
export default class PrivacySection {
  ...
  constructor(
    ...
    this.radioSection = new SettingSection({
      name: options.title,
      caption: true,
    });
    ...
    const form = RadioFormFromRows(
      [...this.radioRows.values()],
      this.onRadioChange
    );
    ...
    if (!options.noExceptions) {
      const section = (this.exceptionsSection = new SettingSection({
              ...
              options.tab.slider.createTab(AppAddMembersTab).open({
                ...
                takeOut: (newPeerIds) => {
                  ...
                  exception.row.subtitle.replaceChildren(
                    ...this.generateStr(this.splitPeersByType(newPeerIds))
                  );
    ...
    const promise = (
      options.inputKey
        ? managers.appPrivacyManager.getPrivacy(options.inputKey)
        : Promise.resolve()
    ).then((rules) => {
      ...
      if (this.exceptions) {
        ...
        ["allow" as const, "disallow" as const].forEach((k) => {
          const arr = [];
          const from =
            k === "allow" ? details.allowPeers : details.disallowPeers;
      ...
      options.tab.eventListener.addEventListener("destroy", this.onTabDestroy, {
        once: true,
      });
    });
  }
  ...
  public onTabDestroy = async () => {
        ...
        if (splitted.users.length) {
          rules.push({
            ...
            users: await Promise.all(
              splitted.users.map((id) =>
                rootScope.managers.appUsersManager.getUserInput(id)
              )
            ),
          });
        }
    ...
    rootScope.managers.appPrivacyManager.setPrivacy(
      this.options.inputKey,
      rules
    );
  };

  private replaceCaption(
    caption: PrivacySectionStr = this.isLocked()
      ? this.options.premiumCaption
      : this.options.captions[this.type]
  ) {
  ...
  private splitPeersByType(peerIds: PeerId[]) {
    ...
    peerIds.forEach((peerId) => {
      peers[peerId.isAnyChat() ? "chats" : "users"].push(
        peerId.isAnyChat() ? peerId.toChatId() : peerId
      );
    });
